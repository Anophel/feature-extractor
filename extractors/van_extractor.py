...
class VANExtractor(Extractor):
    """
    Extractor that feeds forward the images through VAN.
    """
    ...
    def __init__(self, size : str = "base") -> None:
        ...
        if size in supported_sizes:
            source = f"Visual-Attention-Network/van-{size}"
        else:
        ...
        self.model = VanModel.from_pretrained(source, output_hidden_states=True)
