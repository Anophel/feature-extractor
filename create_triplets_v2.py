...
def main(args):
    ...
    videos_list = None
    if "videos_filter" in config and type(config["videos_filter"]) is str and not config["videos_filter"]:
                    ...
                    for closer_class in distance_classes:
                        for farther_class in filter(lambda c: c >= closer_class, distance_classes):
                            ...
                            generated_triplets.append(Triplet(prefix, image_list[target_idx], image_list[closer_idx], 
                                ...
                                distances[farther_idx], np.where(sorted_indexes == closer_idx)[0][0], np.where(sorted_indexes == farther_idx)[0][0], 
                                options_distance, closer_class, farther_class, {}))
